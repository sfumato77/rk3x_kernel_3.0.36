...
s32 gtp_fw_startup(struct i2c_client *client)
{
    ...
    //init sw WDT
	opr_buf[0] = 0xAA;
//mia	ret = i2c_write_bytes(client, 0x8041, opr_buf, 1);
    if (ret < 0)
    {
        return FAIL;
    }
    ...
    //release SS51 & DSP
    opr_buf[0] = 0x00;
//mia    ret = i2c_write_bytes(client, 0x4180, opr_buf, 1);
    if (ret < 0)
    {
        return FAIL;
    }
    ...
    //check fw run status
//mia    ret = i2c_read_bytes(client, 0x8041, opr_buf, 1);
    if (ret < 0)
    {
        return FAIL;
    }
    ...
    else
    {
        ...
        opr_buf[0] = 0xAA;
//mia        i2c_write_bytes(client, 0x8041, opr_buf, 1);
        return SUCCESS;
    }
}
...
static s32 gtp_bak_ref_proc(struct goodix_ts_data *ts, u8 mode)
{
    ...
    //mia    ret = i2c_write_bytes(ts->client, GTP_REG_BAK_REF, p_bak_ref, ts->bak_ref_len);
        if (FAIL == ret)
        {
...
//mia        ret = i2c_read_bytes(ts->client, GTP_REG_BAK_REF, p_bak_ref, ts->bak_ref_len);
        if (ret < 0)
        {
...
//	mdelay(200);
//	printk("\n\n\nI am now here_____________________________\n\n\n");
