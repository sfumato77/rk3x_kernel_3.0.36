...
#define WAKE_PORT			RK30_PIN4_PD0//NULL 
...
//	rk30_mux_api_set(GPIO4C2_SMCDATA2_TRACEDATA2_NAME, GPIO4C_GPIO4C2); //ljy add 121105
...
static u32 gsl_read_interface(struct i2c_client *client, u8 reg, u8 *buf, u32 num)
{
	...
	xfer_msg[0].buf = &reg;
       xfer_msg[0].scl_rate=200000;

	xfer_msg[1].addr = client->addr;
	xfer_msg[1].len = num;
	xfer_msg[1].flags |= I2C_M_RD;
	xfer_msg[1].buf = buf;
       xfer_msg[1].scl_rate=200000;
...
static void report_data(struct gsl_ts *ts, u16 x, u16 y, u8 pressure, u8 id)
{
	...
	#if (defined(CONFIG_TCHIP_MACH_TR830)||defined(CONFIG_TCHIP_MACH_TR832)||defined(CONFIG_TCHIP_MACH_TR730))
			    x = SCREEN_MAX_X  - x;
			    y = SCREEN_MAX_Y - y;
        #endif	
	//printk("x2= %d, y2=%d\n",x,y);
...
extern int gt811_probe_success;
static int __init gsl_ts_init(void)
{
    int ret;
	printk("==gsl_ts_init==\n");
	if(gt811_probe_success == 1)
	{
		printk("==gsl_ts_init== gt811_probe_success exit\n");
		return 0;
	}
...
static void __exit gsl_ts_exit(void)
{
	...
	if(gt811_probe_success == 1)
	{
		printk("==gsl_ts_exit== gt811_probe_success exit\n");
		return 0;
	}
