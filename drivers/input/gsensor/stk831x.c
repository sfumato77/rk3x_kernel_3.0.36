 ...
 *
 *  Copyright (C) 2011~2012 Lex Hsieh / sensortek, Sitronix 
...
#define STK_ACC_DRIVER_VERSION	"1.5.5"
/*choose polling or interrupt mode*/
#define STK_ACC_POLLING_MODE	1// 1
#if (!STK_ACC_POLLING_MODE)
	#define ADDITIONAL_GPIO_CFG 1
	#define STK_INT_PIN	RK30_PIN4_PC0 //39
#endif
...
#if STK_ACC_POLLING_MODE
		int ret;
		printk("%s:%d: wait_for_completion...\n", __func__, __LINE__);
		atomic_set(&stk->run_thread, 0);
		ret = wait_for_completion_timeout(&acc_thread_completion, msecs_to_jiffies(100));
		acc_polling_tsk = NULL;
		printk("%s:%d: wait_for_completion done! ret = %d\n", __func__, __LINE__, ret);
#else
...
//	w_buf[1] = STK_ACC_CALI_VER1;
	...
	else
	{
		...
		for(i=0;i<STK_ACC_CALI_FILE_SIZE;i++)
		{
			if(r_buf[i] != w_buf[i])
			{
				printk(KERN_ERR "%s: read back error!\n", __func__);
				filp_close(cali_file,NULL);
				return -EIO;
			}
		}
...
MODULE_AUTHOR("Lex Hsieh / SenseTek");
